...
namespace ThirdPersonExample
{
	...
	// - Escape key: stops camera movement so that you can (for example) fiddle around with settings in Aspectnut.
	...
	public class PlayerController : Entity
	{
		...
		protected override void OnUpdate(float ts)
		{
			...
			if (m_CharacterController != null)
			{
				...
				if (m_CharacterController.IsGrounded && Input.IsKeyDown(KeyCode.Space))
				{
	...
	public class PlayerControllerAnimated : Entity
	{
		...
		protected override void OnUpdate(float ts)
		{
			...
			if (keyDown)
			{
				shiftDown = Input.IsKeyDown(KeyCode.LeftShift);
			...
			if (m_CharacterController.IsGrounded || m_CharacterController.SpeedDown < (shiftDown ? 4.0f : 2.0f))
			{
				m_Animation.StateIndex = keyDown ? Input.IsKeyDown(KeyCode.LeftShift) ? 2u : 1u : 0u;
