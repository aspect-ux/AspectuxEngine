...
namespace AudioTest
{
	...
	public class SpreadTest : Entity
	{
		...
		protected override void OnUpdate(float ts)
		{
			...
			LeftChannel.Translation = GetPositionOnCircumference(Mathf.Deg2Rad * angle, -radius);
			RightChannel.Translation = GetPositionOnCircumference(Mathf.Deg2Rad * angle, radius);
	...
	public class SpatializerStress : Entity
	{
		...
		protected override void OnCreate()
		{
			...
			{ 
				...
				{
					float x = SpawnOffset.X + Aspect.Random.Float() * SpawnRange.X;
					float y = SpawnOffset.Y + Aspect.Random.Float() * SpawnRange.Y;
					float rotation = Aspect.Random.Float() * 10.0f;
