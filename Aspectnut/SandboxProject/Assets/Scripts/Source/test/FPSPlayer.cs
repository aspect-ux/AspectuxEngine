...
namespace FPSExample
{
	public class FPSPlayer : Entity
	{
		...
		private Aspect.Random rand = new Aspect.Random();
		...
		{
			float rv = Aspect.Random.Float() * 0.06f;
			float rp = Aspect.Random.Float() * 0.3f;
		...
		private void UpdateRaycasting()
		{
			...
			{
					if (Input.IsKeyPressed(KeyCode.H) && Physics.Raycast(m_RaycastData, out hitInfo))
					{
			...
			{
				if (Input.IsKeyPressed(KeyCode.H) && Physics.Raycast(m_CameraTransform.Translation + m_CameraTransform.WorldTransform.Forward * 2.0f, m_CameraTransform.WorldTransform.Forward, 50.0f, out hitInfo, typeof(MeshComponent)))
				{
					Entity entity = Scene.FindEntityByID(hitInfo.EntityID);
					TagComponent tag = entity.GetComponent<TagComponent>();
					MeshComponent mesh = entity.GetComponent<MeshComponent>();
