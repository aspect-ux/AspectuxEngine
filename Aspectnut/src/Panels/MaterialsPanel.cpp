...
namespace Aspect {
	...
	void MaterialsPanel::SetSceneContext(const AspectRef<Scene>& context)
	{
		m_Context = context;
	}
	...
	void MaterialsPanel::OnImGuiRender(bool& isOpen)
	{
		...
		//TODO: has mesh component
		const bool hasValidEntity = true;// = m_SelectedEntity && m_SelectedEntity.HasAny<MeshComponent, StaticMeshComponent>();

		ImGui::SetNextWindowSize(ImVec2(200.0f, 300.0f), ImGuiCond_Appearing);
		if (ImGui::Begin("Materials", &isOpen) && hasValidEntity)
		{
			/* TODO: Dynamic and static 
			const bool hasDynamicMesh = m_SelectedEntity.HasComponent<MeshComponent>() && AssetManager::IsAssetHandleValid(m_SelectedEntity.GetComponent<MeshComponent>().m_Mesh);
			...
			const bool hasMesh = m_SelectedEntity.HasComponent<MeshComponent>();

			if (hasMesh/*hasDynamicMesh || hasStaticMesh*/)
			{
				AspectRef<MaterialTable> meshMaterialTable, componentMaterialTable;
				...
				if (m_SelectedEntity.HasComponent<MeshComponent>())
				{
					...
					/*auto mesh = AssetManager::GetAsset<Mesh>(meshComponent.m_Mesh);
					if (mesh)
				...
				//AS_CORE_VERIFY(meshMaterialTable != nullptr && componentMaterialTable != nullptr);
	...
	void MaterialsPanel::RenderMaterial(size_t materialIndex, AssetHandle materialAssetHandle)
	{
		...
		Ref<Shader> transparentShader = Renderer::GetShaderLibrary()->Get("AspectPBR_Transparent");
		...
		if (!transparent)
		{
			...
			}
			{
				...
				if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
				{
					...
					if (ImGui::SliderFloat("Metalness Value##MetalnessInput", &metalnessValue, 0.0f, 1.0f))
