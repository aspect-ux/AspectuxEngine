...
namespace Aspect {
	...
	static Aspect::AllocationStats s_GlobalStats;
...
#ifndef AS_DIST
			if (!found)
				1;//TODO: AS_CORE_WARN_TAG("Memory", "Memory block {0} not present in alloc map", memory);
#endif
		}
...
{
	return Aspect::Allocator::Allocate(size);
}
...
void* __CRTDECL operator new[](size_t size)
	...
	{
		return Aspect::Allocator::Allocate(size, desc);
	}
	...
	_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
		...
		{
			return Aspect::Allocator::Allocate(size, file, line);
		}
		...
		_NODISCARD _Ret_notnull_ _Post_writable_byte_size_(size) _VCRT_ALLOCATOR
			...
			{
				return Aspect::Allocator::Free(memory);
			}
