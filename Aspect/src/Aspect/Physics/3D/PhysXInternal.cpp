...
namespace Aspect {
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		case physx::PxErrorCode::eDEBUG_INFO:
			AS_CORE_INFO_TAG("Physics", "{0} at {1} ({2})", s_PhysXData->LastErrorMessage, file, line);
			break;
		case physx::PxErrorCode::eDEBUG_WARNING:
		case physx::PxErrorCode::ePERF_WARNING:
			AS_CORE_WARN_TAG("Physics", "{0} at {1} ({2})", s_PhysXData->LastErrorMessage, file, line);
			break;
		case physx::PxErrorCode::eINVALID_PARAMETER:
		case physx::PxErrorCode::eINVALID_OPERATION:
		case physx::PxErrorCode::eOUT_OF_MEMORY:
		case physx::PxErrorCode::eINTERNAL_ERROR:
			AS_CORE_ERROR_TAG("Physics", "{0} at {1} ({2})", s_PhysXData->LastErrorMessage, file, line);
			AS_CORE_ASSERT(false);
			break;
		case physx::PxErrorCode::eABORT:
		case physx::PxErrorCode::eMASK_ALL:
			AS_CORE_FATAL_TAG("Physics", "{0} at {1} ({2})", s_PhysXData->LastErrorMessage, file, line);
			AS_CORE_ASSERT(false);
			break;
		}
	}
	...
	{
		AS_CORE_ERROR_TAG("Physics", "{0}:{1} - {2}", file, line, exp);
		AS_CORE_ASSERT(false);
	}

	void PhysXInternal::Initialize()
	{
		AS_CORE_ASSERT(!s_PhysXData, "Trying to initialize the PhysX SDK multiple times!");
		...
		AS_CORE_ASSERT(s_PhysXData->PhysXFoundation, "PxCreateFoundation failed.");
		...
		AS_CORE_ASSERT(s_PhysXData->PhysXSDK, "PxCreatePhysics failed.");

		bool extentionsLoaded = PxInitExtensions(*s_PhysXData->PhysXSDK, PhysXDebugger::GetDebugger());
		AS_CORE_ASSERT(extentionsLoaded, "Failed to initialize PhysX Extensions.");

		s_PhysXData->PhysXCPUDispatcher = physx::PxDefaultCpuDispatcherCreate(1);

		//TODO:CookingFactory::Initialize();
