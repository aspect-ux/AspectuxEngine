...
//#include "Aspect/Script/ScriptUtils.h"
...
namespace Aspect {
	...
	void PhysicsActor::AddForce(const glm::vec3& force, ForceMode forceMode)
	{
		...
		{
			AS_CORE_WARN("Trying to add force to a non-dynamic or kinematic PhysicsActor.");
			return;
		}
	...
	void PhysicsActor::AddTorque(const glm::vec3& torque, ForceMode forceMode)
	{
		if (!IsDynamic() || IsKinematic())
		{
			AS_CORE_WARN("Trying to add torque to a non-dynamic or kinematic PhysicsActor.");
			return;
		}
	...
	glm::vec3 PhysicsActor::GetLinearVelocity() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get velocity of non-dynamic PhysicsActor.");
	...
	void PhysicsActor::SetLinearVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	glm::vec3 PhysicsActor::GetAngularVelocity() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get angular velocity of non-dynamic PhysicsActor.");
			return glm::vec3(0.0f);
		}
	...
	void PhysicsActor::SetAngularVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set angular velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	float PhysicsActor::GetMaxLinearVelocity() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get max linear velocity of non-dynamic PhysicsActor.");
			return 0.0f;
		}
	...
	void PhysicsActor::SetMaxLinearVelocity(float maxVelocity)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set max linear velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	float PhysicsActor::GetMaxAngularVelocity() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get max angular velocity of non-dynamic PhysicsActor.");
			return 0.0f;
		}
	...
	void PhysicsActor::SetMaxAngularVelocity(float maxVelocity)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set max angular velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	float PhysicsActor::GetLinearDrag() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get linear drag of non-dynamic PhysicsActor.");
			return 0.0f;
		}
	...
	void PhysicsActor::SetLinearDrag(float drag)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set linear drag of non-dynamic PhysicsActor.");
			return;
		}
	...
	float PhysicsActor::GetAngularDrag() const
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to get angular drag of non-dynamic PhysicsActor.");
			return 0.0f;
		}
	...
	void PhysicsActor::SetAngularDrag(float drag)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Trying to set angular drag of non-dynamic PhysicsActor.");
			return;
		}
	...
	glm::vec3 PhysicsActor::GetKinematicTargetPosition() const
	{
		if (!IsKinematic())
		{
			AS_CORE_WARN("Trying to get kinematic target for a non-kinematic actor.");
		...
		AS_CORE_VERIFY(actor->getKinematicTarget(target), "kinematic target not set");
	...
	void PhysicsActor::SetKinematicTarget(const glm::vec3& targetPosition, const glm::quat& targetRotation) const
	{
		if (!IsKinematic())
		{
			AS_CORE_WARN("Trying to set kinematic target for a non-kinematic actor.");
			return;
		}
	...
	void PhysicsActor::SetKinematic(bool isKinematic)
	{
		if (!IsDynamic())
		{
			AS_CORE_WARN("Static PhysicsActor can't be kinematic.");
			return;
		}
	...
	void PhysicsActor::AddCollider(MeshColliderComponent& collider)
	{
		...
		AS_CORE_VERIFY(colliderAsset);

		auto& meshCache = PhysicsSystem::GetMeshCache();

		if (!meshCache.Exists(colliderAsset))
		{
			AS_CORE_WARN_TAG("Physics", "Tried to add an Mesh Collider with an invalid collider asset. Please make sure your collider has been cooked.");
			return;
		}
		...
		{
			AS_CONSOLE_LOG_ERROR("Entity '{0}' ({1}) has a dynamic RigidBodyComponent along with a Complex MeshColliderComponent! This isn't allowed.", m_Entity.Name(), m_Entity.GetUUID());
			return;
		}
	...
	void PhysicsActor::CreateRigidActor()
	{
		...
		AspectRef<Scene> scene = Scene::GetScene(m_Entity.GetSceneUUID());
