...
namespace Aspect
{
	...
	void FileSystem::StartWatching()
	{
		...
		AS_CORE_ASSERT(s_WatcherThread != NULL);
		SetThreadDescription(s_WatcherThread, L"Aspect FileSystemWatcher");
	}
	...
	unsigned long FileSystem::Watch(void* param)
	{
		...
		{
			AS_CORE_VERIFY(false, "Failed to open directory!");
			return 0;
		}
		...
		while (s_Watching && result)
		{
			...
			do
			{
				...
				// NOTE(Peter): Fix for https://gitlab.com/chernoprojects/Aspect-dev/-/issues/143
	...
	std::filesystem::path FileSystem::GetPersistentStoragePath()
	{
		...
		AS_CORE_VERIFY(result == S_OK);
		s_PersistentStoragePath = roamingFilePath;
		s_PersistentStoragePath /= "Aspectnut";
