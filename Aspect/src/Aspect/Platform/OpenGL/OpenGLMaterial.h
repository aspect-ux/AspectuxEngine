...
namespace Aspect {
	...
	public:
		...
		OpenGLMaterial(AspectRef<Material> material, const std::string& name = "");
		...
		virtual Ref<TextureCube> TryGetTextureCube(const std::string& name) override { return nullptr; }

		template <typename T>
		void Set(const std::string& name, const T& value)
		{
			auto decl = FindUniformDeclaration(name);
			AS_CORE_ASSERT(decl, "Could not find uniform!");
			if (!decl)
				return;
		...
		{
			auto decl = FindUniformDeclaration(name);
			AS_CORE_ASSERT(decl, "Could not find uniform with name 'x'");
			auto& buffer = m_UniformStorageBuffer;
		...
		Ref<T> GetResource(const std::string& name)
		{
			...
			AS_CORE_ASSERT(slot < m_Textures.size(), "Texture slot is invalid!");
	...
	private:
		...
		/*void SetOpenGLDescriptor(const std::string& name, const Ref<Texture2D>& texture);
		void SetOpenGLDescriptor(const std::string& name, const Ref<Texture2D>& texture, uint32_t arrayIndex);
		void SetOpenGLDescriptor(const std::string& name, const Ref<TextureCube>& texture);
		void SetOpenGLDescriptor(const std::string& name, const Ref<Image2D>& image);*/
	...
	private:
		...
		*/
		...
		//OpenGLShader::ShaderMaterialDescriptorSet m_DescriptorSets[3];
