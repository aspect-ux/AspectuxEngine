...
namespace Aspect::Utils {
	...
	public:
		...
		ValueWrapper(const ValueWrapper& other)
		{
			...
			if (m_Size)
			{
				m_ValueBuffer = anew byte[m_Size];
		...
		{
			AS_CORE_VERIFY(HasValue(), "ValueWrapper::Get - No value!");
		...
		{
			AS_CORE_VERIFY(HasValue(), "Trying to set the value of an empty ValueWrapper!");
			...
			if constexpr (std::is_same<TValueType, std::string>::value)
			{
				...
				{
					adelete[] m_ValueBuffer;
					m_ValueBuffer = anew byte[value.size()];
					m_Size = value.size();
				}
			...
			{
				AS_CORE_VERIFY(m_Size == sizeof(value));
		...
		{
			ValueWrapper value;
			value.m_ValueBuffer = anew byte[size];
			value.m_Size = size;
			return value;
		}
