...
#include "Aspect/Utilities/YAMLSerializationHelpers.h"
...
namespace Aspect {
	...
	{
		s_MethodThunks = anew MethodThunkCache();

#define REGISTER_METHOD_THUNK(var, method) s_MethodThunks->var.SetThunkFromMethod(method);
		
		REGISTER_METHOD_THUNK(Entity_OnCreate, AS_CACHED_METHOD("Aspect.Entity", "OnCreate", 0));
		REGISTER_METHOD_THUNK(Entity_OnUpdate, AS_CACHED_METHOD("Aspect.Entity", "OnUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnPhysicsUpdate, AS_CACHED_METHOD("Aspect.Entity", "OnPhysicsUpdate", 1));
		REGISTER_METHOD_THUNK(Entity_OnDestroyInternal, AS_CACHED_METHOD("Aspect.Entity", "OnDestroyInternal", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnInstantiate, AS_CACHED_METHOD("Aspect.IEditorRunnable", "OnInstantiate", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnUIRender, AS_CACHED_METHOD("Aspect.IEditorRunnable", "OnUIRender", 0));
		REGISTER_METHOD_THUNK(IEditorRunnable_OnShutdown, AS_CACHED_METHOD("Aspect.IEditorRunnable", "OnShutdown", 0));
...
#undef REGISTER_METHOD_THUNK
	...
	bool ScriptUtils::CheckMonoError(MonoError& error)
	{
		...
		if (hasError)
		{
			...
			AS_CORE_ERROR_TAG("ScriptEngine", "Mono Error!");
			AS_CORE_ERROR_TAG("ScriptEngine", "\tError Code: {0}", errorCode);
			AS_CORE_ERROR_TAG("ScriptEngine", "\tError Message: {0}", errorMessage);
	...
	void ScriptUtils::HandleException(MonoObject* exception)
	{
		...
		AS_CONSOLE_LOG_ERROR("{0}: {1}. Source: {2}, Stack Trace: {3}", exceptionInfo.TypeName, exceptionInfo.Message, exceptionInfo.Source, exceptionInfo.StackTrace);
	}

	Buffer ScriptUtils::GetFieldValue(MonoObject* classInstance, const std::string& fieldName, FieldType fieldType, bool isProperty)
	{
		AS_PROFILE_FUNC();
		return MonoObjectToValue(GetFieldValueObject(classInstance, fieldName, isProperty), fieldType);
	}

	MonoObject* ScriptUtils::GetFieldValueObject(MonoObject* classInstance, const std::string& fieldName, bool isProperty)
	{
		AS_PROFILE_FUNC();
	...
	MonoObject* ScriptUtils::ValueToMonoObject(const void* data, FieldType dataType)
	{
		...
		else
		{
			switch (dataType)
			{
			case FieldType::String: return (MonoObject*)UTF8StringToMono(*(std::string*)data);
			//case FieldType::Prefab: return ScriptEngine::CreateManagedObject("Aspect.Prefab", *(AssetHandle*)data);
			case FieldType::Entity: return ScriptEngine::CreateManagedObject("Aspect.Entity", *(UUID*)data);
			/*case FieldType::Mesh: return ScriptEngine::CreateManagedObject("Aspect.Mesh", *(AssetHandle*)data);
			case FieldType::StaticMesh: return ScriptEngine::CreateManagedObject("Aspect.StaticMesh", *(AssetHandle*)data);
			case FieldType::Material: return ScriptEngine::CreateManagedObject("Aspect.Material", *(AssetHandle*)data);
			case FieldType::PhysicsMaterial: return ScriptEngine::CreateManagedObject("Aspect.PhysicsMaterial", *(AssetHandle*)data);
			case FieldType::Texture2D: return ScriptEngine::CreateManagedObject("Aspect.Texture2D", *(AssetHandle*)data);*/
			}
		}

		AS_CORE_ASSERT(false, "Unsupported value type!");
		return nullptr;
	}
	...
	/*MonoObject* ScriptUtils::GetDefaultValueObjectForType(const ManagedType& type)
	{
		...
		switch (type.NativeType)
		{
			case UnmanagedType::Bool: return BoxValue(AS_CACHED_CLASS("System.Boolean"), (MonoBoolean)false);
			case UnmanagedType::Int8: return BoxValue(AS_CACHED_CLASS("System.SByte"), 0);
			case UnmanagedType::Int16: return BoxValue(AS_CACHED_CLASS("System.Int16"), 0);
			case UnmanagedType::Int32: return BoxValue(AS_CACHED_CLASS("System.Int32"), 0);
			case UnmanagedType::Int64: return BoxValue(AS_CACHED_CLASS("System.Int64"), 0);
			case UnmanagedType::UInt8: return BoxValue(AS_CACHED_CLASS("System.Byte"), 0);
			case UnmanagedType::UInt16: return BoxValue(AS_CACHED_CLASS("System.UInt16"), 0);
			case UnmanagedType::UInt32: return BoxValue(AS_CACHED_CLASS("System.UInt32"), 0);
			case UnmanagedType::UInt64: return BoxValue(AS_CACHED_CLASS("System.UInt64"), 0);
			case UnmanagedType::Float: return BoxValue(AS_CACHED_CLASS("System.Single"), 0);
			case UnmanagedType::Double: return BoxValue(AS_CACHED_CLASS("System.Double"), 0);
			case UnmanagedType::String: return (MonoObject*)UTF8StringToMono("");
			case UnmanagedType::AssetHandle: return BoxValue(AS_CACHED_CLASS("Aspect.AssetHandle"), 0);
			case UnmanagedType::Vector2: return BoxValue(AS_CACHED_CLASS("Aspect.Vector2"), glm::vec2(0.0f));
			case UnmanagedType::Vector3: return BoxValue(AS_CACHED_CLASS("Aspect.Vector3"), glm::vec3(0.0f));
			case UnmanagedType::Vector4: return BoxValue(AS_CACHED_CLASS("Aspect.Vector4"), glm::vec4(0.0f));
	...
	FieldType ScriptUtils::GetFieldTypeFromMonoType(MonoType* monoType)
	{
		...
		{
			if (AS_CORE_CLASS(AssetHandle) && typeClass == AS_CORE_CLASS(AssetHandle)->Class)
				return FieldType::AssetHandle;

			if (AS_CORE_CLASS(Vector2) && typeClass == AS_CORE_CLASS(Vector2)->Class)
				return FieldType::Vector2;

			if (AS_CORE_CLASS(Vector3) && typeClass == AS_CORE_CLASS(Vector3)->Class)
				return FieldType::Vector3;

			if (AS_CORE_CLASS(Vector4) && typeClass == AS_CORE_CLASS(Vector4)->Class)
		...
		{
			auto entityClass = AS_CORE_CLASS(Entity);

			if (entityClass && mono_class_is_assignable_from(typeClass, entityClass->Class))
				return FieldType::Entity;
			/*
			if (AS_CORE_CLASS(Prefab) && typeClass == AS_CORE_CLASS(Prefab)->Class)
				return FieldType::Prefab;

			if (AS_CORE_CLASS(Mesh) && typeClass == AS_CORE_CLASS(Mesh)->Class)
				return FieldType::Mesh;

			if (AS_CORE_CLASS(StaticMesh) && typeClass == AS_CORE_CLASS(StaticMesh)->Class)
				return FieldType::StaticMesh;

			if (AS_CORE_CLASS(Material) && typeClass == AS_CORE_CLASS(Material)->Class)
				return FieldType::Material;

			if (AS_CORE_CLASS(PhysicsMaterial) && typeClass == AS_CORE_CLASS(PhysicsMaterial)->Class)
				return FieldType::PhysicsMaterial;

			if (AS_CORE_CLASS(Texture2D) && typeClass == AS_CORE_CLASS(Texture2D)->Class)
	...
	//{
	//	AS_CORE_VERIFY(arr, "Can't get a value from a nullptr array");

	//	uintptr_t length = mono_array_length(arr);
	//	AS_CORE_VERIFY(index < length, "index out of range");
	...
	//{
	//	AS_CORE_VERIFY(arr && *arr, "Can't resize a nullptr array");
	//	AS_CORE_VERIFY(newLength > 0, "Can't resize an array to a length of 0");

	//	uintptr_t length = mono_array_length(*arr);
	//	AS_CORE_VERIFY(length > 0, "Cannot resize an array of length 0");

	//	uintptr_t lengthToCopy = newLength < length ? newLength : length;

	//	MonoClass* arrayClass = mono_object_get_class((MonoObject*)*arr);
	//	MonoClass* elementClass = mono_class_get_element_class(arrayClass);
	//	int32_t elementSize = mono_array_element_size(arrayClass);

	//	MonoArray* temp = mono_array_new(ScriptEngine::GetScriptDomain(), elementClass, newLength);
	//	AS_CORE_VERIFY(temp);

	//	char* src = mono_array_addr_with_size(*arr, elementSize, 0);
	//	char* dst = mono_array_addr_with_size(temp, elementSize, 0);
	//	memcpy(dst, src, lengthToCopy * elementSize);
	...
	//{
	//	AS_CORE_VERIFY(arr && *arr, "Cannot remove elements from nullptr array");
	...
	{
		AS_CORE_VERIFY(!arrayClass.empty(), "Cannot create managed array of no type");

		ManagedClass* klass = ScriptCache::GetManagedClassByName(arrayClass);
		AS_CORE_VERIFY(klass, "Unable to find array class");
	...
	{
		AS_CORE_VERIFY(arrayClass, "Cannot create managed array of no type");
	...
	void ManagedArrayUtils::SetValueInternal(MonoArray* arr, uintptr_t index, void* data)
	{
		...
		if (index >= length)
		{
			AS_CORE_WARN_TAG("ScriptEngine", "Index out of bounds in C# array!");
			return;
		}
