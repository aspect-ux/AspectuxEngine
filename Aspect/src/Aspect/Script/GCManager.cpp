...
namespace Aspect {
	...
	void GCManager::Init()
	{
		AS_CORE_ASSERT(!s_GCState, "Trying to initialize GC Manager multiple times!");

		s_GCState = anew GCState();
	}
	...
	void GCManager::Shutdown()
	{
		...
		{
			AS_CORE_ERROR("ScriptEngine", "Memory leak detected!");
			AS_CORE_ERROR("ScriptEngine", "Not all GCHandles have been cleaned up!");
	...
	{
		AS_PROFILE_FUNC();
		AS_CORE_INFO_TAG("ScriptEngine", "Collecting garbage...");
		mono_gc_collect(mono_gc_max_generation());
		if (blockUntilFinalized)
		{
			while (mono_gc_pending_finalizers());
			AS_CORE_INFO_TAG("ScriptEngine", "GC Finished...");
		}
	}
	...
	GCHandle GCManager::CreateObjectReference(MonoObject* managedObject, bool weakReference, bool pinned, bool track)
	{
		...
		AS_CORE_ASSERT(handle, "Failed to retrieve valid GC Handle!");
	...
	void GCManager::ReleaseObjectReference(GCHandle handle)
	{
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Tried to release an object reference using an invalid handle!");
			return;
		}
