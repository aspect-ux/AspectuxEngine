...
namespace Aspect {
	...
	{
		AS_CORE_ASSERT(!s_Cache, "Trying to initialize ScriptCache multiple times!");
		s_Cache = anew Cache();
	...
	{
		AS_CORE_INFO("Start Caching Class {0},MonoClass is not null {1}", className,monoClass!=nullptr);
		...
		if (managedClass.FullName.find("Aspect.") != std::string::npos)
		{
			AspectRef<AssemblyInfo> coreAssembly = ScriptEngine::GetCoreAssemblyInfo();
...
#define CACHE_Aspect_CORE_CLASS(name) CacheClass("Aspect." ##name, mono_class_from_name(ScriptEngine::GetCoreAssemblyInfo()->AssemblyImage, "Aspect", name))

		
		CACHE_Aspect_CORE_CLASS("ShowInEditorAttribute");
		CACHE_Aspect_CORE_CLASS("HideFromEditorAttribute");
		CACHE_Aspect_CORE_CLASS("ClampValueAttribute");
		
		CACHE_Aspect_CORE_CLASS("AssetHandle");
		CACHE_Aspect_CORE_CLASS("Vector2");
		CACHE_Aspect_CORE_CLASS("Vector3");
		CACHE_Aspect_CORE_CLASS("Vector4");
		CACHE_Aspect_CORE_CLASS("Entity");
		CACHE_Aspect_CORE_CLASS("IEditorRunnable");
		CACHE_Aspect_CORE_CLASS("Prefab");
		CACHE_Aspect_CORE_CLASS("Mesh");
		CACHE_Aspect_CORE_CLASS("StaticMesh");
		CACHE_Aspect_CORE_CLASS("Material");
		CACHE_Aspect_CORE_CLASS("Collider");
		CACHE_Aspect_CORE_CLASS("BoxCollider");
		CACHE_Aspect_CORE_CLASS("SphereCollider");
		CACHE_Aspect_CORE_CLASS("CapsuleCollider");
		CACHE_Aspect_CORE_CLASS("MeshCollider");
		CACHE_Aspect_CORE_CLASS("PhysicsMaterial");
		CACHE_Aspect_CORE_CLASS("RaycastHit2D");
		CACHE_Aspect_CORE_CLASS("Texture2D");
	}
	...
	static void BuildClassMetadata(AspectRef<AssemblyInfo>& assemblyInfo, MonoClass* monoClass)
	{
		AS_CORE_VERIFY(monoClass);
		...
		if (parentClass != nullptr && parentClass != AS_CACHED_CLASS_RAW("System.Object"))
		{
	...
	void ScriptCache::GenerateCacheForAssembly(AspectRef<AssemblyInfo> assemblyInfo)
	{
		...
		for (auto classID : assemblyInfo->Classes)
		{
			...
			AS_CORE_ERROR("classID{0}", classID);
			...
			if (mono_class_is_subclass_of(managedClass.Class, AS_CACHED_CLASS_RAW("Aspect.Entity"), false))
			{
	...
	MonoClass* ScriptCache::GetFieldTypeClass(FieldType fieldType)
	{
		switch (fieldType)
		{
		case FieldType::Bool: return AS_CACHED_CLASS("System.Bool")->Class;
		case FieldType::Int8: return AS_CACHED_CLASS("System.SByte")->Class;
		case FieldType::Int16: return AS_CACHED_CLASS("System.Int16")->Class;
		case FieldType::Int32: return AS_CACHED_CLASS("System.Int32")->Class;
		case FieldType::Int64: return AS_CACHED_CLASS("System.Int64")->Class;
		case FieldType::UInt8: return AS_CACHED_CLASS("System.Byte")->Class;
		case FieldType::UInt16: return AS_CACHED_CLASS("System.UInt16")->Class;
		case FieldType::UInt32: return AS_CACHED_CLASS("System.UInt32")->Class;
		case FieldType::UInt64: return AS_CACHED_CLASS("System.UInt64")->Class;
		case FieldType::Float: return AS_CACHED_CLASS("System.Single")->Class;
		case FieldType::Double: return AS_CACHED_CLASS("System.Double")->Class;
		case FieldType::String: return AS_CACHED_CLASS("System.String")->Class;
		case FieldType::Vector2: return AS_CACHED_CLASS("Aspect.Vector2")->Class;
		case FieldType::Vector3: return AS_CACHED_CLASS("Aspect.Vector3")->Class;
		case FieldType::Vector4: return AS_CACHED_CLASS("Aspect.Vector4")->Class;
		case FieldType::AssetHandle: return AS_CACHED_CLASS("Aspect.AssetHandle")->Class;
		case FieldType::Prefab: return AS_CACHED_CLASS("Aspect.Prefab")->Class;
		case FieldType::Entity: return AS_CACHED_CLASS("Aspect.Entity")->Class;
		case FieldType::Mesh: return AS_CACHED_CLASS("Aspect.Mesh")->Class;
		case FieldType::StaticMesh: return AS_CACHED_CLASS("Aspect.StaticMesh")->Class;
		case FieldType::Material: return AS_CACHED_CLASS("Aspect.Material")->Class;
		case FieldType::PhysicsMaterial: return AS_CACHED_CLASS("Aspect.PhysicsMaterial")->Class;
		case FieldType::Texture2D: return AS_CACHED_CLASS("Aspect.Texture2D")->Class;
		}
	...
	ManagedMethod* ScriptCache::GetManagedMethod(ManagedClass* managedClass, const std::string& name, bool ignoreParent /*= false*/)
	{
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Attempting to get method {0} from a nullptr class!", name);
			return nullptr;
		}
		...
		AS_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} in class {1}", name, managedClass->FullName);
		return nullptr;
	}
	...
	ManagedMethod* ScriptCache::GetSpecificManagedMethod(ManagedClass* managedClass, const std::string& name, uint32_t parameterCount, bool ignoreParent)
	{
		...
		if (method == nullptr)
			AS_CORE_WARN_TAG("ScriptEngine", "Failed to find method with name: {0} and parameter count: {1} in class {2}", name, parameterCount, managedClass->FullName);

		return method;
	}

	void ScriptCache::CacheClassMethods(AspectRef<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while ((monoMethod = mono_class_get_methods(managedClass.Class, &methodPtr)) != NULL)
		{
			...
			AS_CORE_INFO("Cache class method: {0}", fullName);
	...
	void ScriptCache::CacheClassFields(AspectRef<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while (currentClass != nullptr)
		{
			...
			if (classNameSpace.find("Aspect") != std::string::npos && className.find("Entity") != std::string::npos)
			{
			...
			while ((field = mono_class_get_fields(currentClass, &fieldPtr)) != NULL)
			{
				...
				if (attributes && mono_custom_attrs_has_attr(attributes, GetManagedClassByName("Aspect.ShowInEditorAttribute")->Class))
				{
					managedField.Flags &= ~(uint64_t)FieldFlag::Protected;
					managedField.Flags &= ~(uint64_t)FieldFlag::Internal;
					managedField.Flags &= ~(uint64_t)FieldFlag::Private;
					...
					MonoObject* attrib = mono_custom_attrs_get_attr(attributes, GetManagedClassByName("Aspect.ShowInEditorAttribute")->Class);
					//AS_TRY_GET_FIELD_VALUE(managedField.DisplayName, "Aspect.ShowInEditorAttribute", "DisplayName", attrib);

					bool isReadOnly = false;
					//AS_TRY_GET_FIELD_VALUE(isReadOnly, "Aspect.ShowInEditorAttribute", "IsReadOnly", attrib);
	...
	void ScriptCache::CacheClassProperties(AspectRef<AssemblyInfo> assemblyInfo, ManagedClass& managedClass)
	{
		...
		while ((prop = mono_class_get_properties(managedClass.Class, &propertyIter)) != NULL)
		{
			...
			if (monoType == nullptr)
			{
				AS_CORE_ERROR_TAG("ScriptEngine", "Failed to retrieve managed type for property '{0}'", name);
				continue;
			}

			MonoCustomAttrInfo* attributes = mono_custom_attrs_from_property(managedClass.Class, prop);

			if (!TypeUtils::ContainsAttribute(attributes, "Aspect.ShowInEditorAttribute"))
				continue;
