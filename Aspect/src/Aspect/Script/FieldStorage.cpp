...
namespace Aspect {
	...
	void ArrayFieldStorage::SetRuntimeArray(const Buffer& data)
	{
		...
		if (mono_type_is_reference(elementType) || mono_type_is_byref(elementType))
		{
			...
			{
				byte* valueData = data.ReadBytes((uint64_t)m_FieldInfo->Size, (uint64_t)(i * m_FieldInfo->Size));
				MonoObject* boxed = ScriptUtils::ValueToMonoObject((const void*)valueData, ScriptUtils::GetFieldTypeFromMonoType(elementType));
		...
		else
		{
			for (uint32_t i = 0; i < m_Length; i++)
			{
				...
				byte* valueData = data.ReadBytes((uint64_t)m_FieldInfo->Size, (uint64_t)(i * m_FieldInfo->Size));
				memcpy(dst, (const void*)valueData, m_FieldInfo->Size);
				adelete[] valueData;
			}
		}
