...
#include "Aspect/Editor/ApplicationSettings.h"
...
namespace Aspect {
	...
	struct ScriptEngineState
	{
		...
		AspectRef<AssemblyInfo> CoreAssemblyInfo = nullptr;
		AspectRef<AssemblyInfo> AppAssemblyInfo = nullptr;
		...
		AspectRef<Scene> SceneContext = nullptr;
		...
		std::unordered_map<UUID, std::unordered_map<uint32_t, AspectRef<FieldStorageBase>>> FieldMap;
	};

	static ScriptEngineState* s_State = nullptr;
	void ScriptEngine::Init(const ScriptEngineConfig& config)
	{
		AS_CORE_ASSERT(!s_State, "[ScriptEngine]: Trying to call ScriptEngine::Init multiple times!");
		s_State = anew ScriptEngineState();
		s_State->Config = config;
		s_State->CoreAssemblyInfo = AspectRef<AssemblyInfo>::Create();
		s_State->AppAssemblyInfo = AspectRef<AssemblyInfo>::Create();
		...
		if (!LoadCoreAssembly())
		{
			...
			AS_CORE_INFO_TAG("ScriptEngine", "Failed to load Aspects C# core, attempting to build automatically using MSBuild");

			auto scriptCoreAssemblyFile = std::filesystem::current_path().parent_path() / "Aspect-ScriptCore" / "Aspect-ScriptCore.csproj";
			ScriptBuilder::BuildCSProject(scriptCoreAssemblyFile);

			if (!LoadCoreAssembly())
				AS_CORE_FATAL_TAG("ScriptEngine", "Failed to build Aspect-ScriptCore! Most likely there were compile errors!");
		}
	}
	...
	{
		AS_PROFILE_FUNC();
		AS_CORE_ASSERT(s_State->SceneContext, "Tring to initialize script runtime without setting the scene context!");
		AS_CORE_ASSERT(s_State->SceneContext->IsPlaying(), "Don't call ScriptEngine::InitializeRuntime if the scene isn't being played!");

		auto view = s_State->SceneContext->GetAllEntitiesWith<IDComponent, ScriptComponent>();
		AS_CORE_WARN("Start InitializeRuntime, {0} ScriptComponent(s) found!", view.size_hint());
		for (auto enttID : view)
		{
			auto uuid = view.get<IDComponent>(enttID).ID;
			Entity entity = s_State->SceneContext->GetEntityWithUUID(uuid);
	...
	void ScriptEngine::ShutdownRuntime()
	{
		...
		AS_CORE_INFO_TAG("ScriptEngine", "Shutting down runtime scene {0}, with {1} scripts active.", runtimeSceneID, s_State->ScriptInstances.size());
	...
	void ScriptEngine::ShutdownRuntimeInstance(Entity entity)
	{
		...
		{
			AspectRef<FieldStorageBase> fieldStorage = s_State->FieldMap[entity.GetUUID()][fieldID];
	...
	bool ScriptEngine::ReloadAppAssembly(const bool scheduleReload)
	{
		...
		AS_CORE_INFO_TAG("ScriptEngine", "Reloading {0}", Project::GetScriptModuleFilePath());
		...
		for (auto& [sceneID, entityMap] : s_State->ScriptEntities)
		{
			...
			for (const auto& entityID : entityMap)
			{
				...
				{
					AspectRef<FieldStorageBase> storage = s_State->FieldMap[entityID][fieldID];
		...
		GCManager::CollectGarbage();
		AS_CORE_INFO_TAG("ScriptEngine", "Done!");
	...
	*/
	bool ScriptEngine::LoadAppAssembly()
	{
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Failed to load app assembly! Invalid filepath");
			AS_CORE_ERROR_TAG("ScriptEngine", "Filepath = {}", Project::GetScriptModuleFilePath());
			return false;
		}
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Failed to load app assembly!");
			return false;
		}
		...
		auto coreMetadataIt = std::find_if(s_State->AppAssemblyInfo->ReferencedAssemblies.begin(), s_State->AppAssemblyInfo->ReferencedAssemblies.end(), [](const AssemblyMetadata& metadata)
			{
				return metadata.Name == "Aspect-ScriptCore";
			});

		if (coreMetadataIt == s_State->AppAssemblyInfo->ReferencedAssemblies.end())
		{
			AS_CONSOLE_LOG_ERROR("C# project doesn't reference Aspect-ScriptCore?");
			return false;
		}
		...
		{
			AS_CONSOLE_LOG_ERROR("C# project referencing an incompatible script core version!");
			AS_CONSOLE_LOG_ERROR("Expected version: {0}.{1}, referenced version: {2}.{3}",
		...
		AS_CORE_INFO_TAG("ScriptEngine", "Successfully loaded app assembly from: {0}", s_State->AppAssemblyInfo->FilePath);

		ScriptGlue::RegisterGlue();
		AS_CORE_INFO("Start Generate Cache for App Assembly");
	...
	void ScriptEngine::SetSceneContext(const AspectRef<Scene>& scene, const Ref<SceneRenderer>& sceneRenderer)
	{
		s_State->SceneContext = scene;
		s_State->ActiveSceneRenderer = sceneRenderer;
	}

	AspectRef<Scene> ScriptEngine::GetSceneContext() { return s_State->SceneContext; }
	...
	void ScriptEngine::InitializeScriptEntity(Entity entity)
	{
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Tried to initialize script entity with an invalid script!");
			return;
		}
		...
		for (auto fieldID : managedClass->Fields)
		{
			...
			if (fieldInfo->IsArray())
				s_State->FieldMap[entityID][fieldID] = AspectRef<ArrayFieldStorage>::Create(fieldInfo);
			else
				s_State->FieldMap[entityID][fieldID] = AspectRef<FieldStorage>::Create(fieldInfo);
	...
	void ScriptEngine::RuntimeInitializeScriptEntity(Entity entity)
	{
		...
		if (!IsModuleValid(scriptComponent.ScriptClassHandle))
		{
			...
			AS_CORE_ERROR_TAG("ScriptEngine", "Tried to instantiate an entity with an invalid C# class '{0}'!", className);
			AS_CONSOLE_LOG_ERROR("Tried to instantiate an entity with an invalid C# class '{0}'!", className);
			return;
		}
		...
		AS_CORE_WARN("Start RUN OnCreate");
	...
	void ScriptEngine::DuplicateScriptInstance(Entity entity, Entity targetEntity)
	{
		...
		if (srcScriptComp.ScriptClassHandle != dstScriptComp.ScriptClassHandle)
		{
			...
			AS_CORE_WARN_TAG("ScriptEngine", "Attempting to duplicate instance of: {0} to an instance of: {1}", srcClass->FullName, dstClass->FullName);
			return;
		}
	...
	AspectRef<FieldStorageBase> ScriptEngine::GetFieldStorage(Entity entity, uint32_t fieldID)
	{
		UUID entityID = entity.GetUUID();
		if (s_State->FieldMap.find(entityID) == s_State->FieldMap.end())
			return nullptr;
	...
	uint32_t ScriptEngine::GetScriptClassIDFromComponent(const ScriptComponent& sc)
	{
		...
		AspectRef<ScriptAsset> scriptAsset = AssetManager::GetAsset<ScriptAsset>(sc.ScriptClassHandle);
	...
	bool ScriptEngine::IsModuleValid(AssetHandle scriptAssetHandle)
	{
		...
		AspectRef<ScriptAsset> scriptAsset = AssetManager::GetAsset<ScriptAsset>(scriptAssetHandle);
	...
	void ScriptEngine::InitMono()
	{
		...
		s_State->RootDomain = mono_jit_init("AspectJITRuntime");
		AS_CORE_ASSERT(s_State->RootDomain, "Unable to initialize Mono JIT");
		...
		AS_CORE_INFO_TAG("ScriptEngine", "Initialized Mono");
	}
	...
	void ScriptEngine::ShutdownMono()
	{
		if (!s_State->IsMonoInitialized)
		{
			AS_CORE_WARN_TAG("ScriptEngine", "Trying to shutdown Mono multiple times!");
			return;
		}
	...
	AspectRef<AssemblyInfo> ScriptEngine::GetCoreAssemblyInfo() { return s_State->CoreAssemblyInfo; }
	AspectRef<AssemblyInfo> ScriptEngine::GetAppAssemblyInfo() { return s_State->AppAssemblyInfo; }
	...
	MonoObject* ScriptEngine::CreateManagedObject(ManagedClass* managedClass)
	{
		...
		AS_CORE_VERIFY(monoObject, "Failed to create MonoObject!");
		return monoObject;
	}
	...
	MonoAssembly* ScriptEngine::LoadMonoAssembly(const std::filesystem::path& assemblyPath)
	{
		...
		if (status != MONO_IMAGE_OK)
		{
			...
			AS_CORE_ERROR_TAG("ScriptEngine", "Failed to open C# assembly '{0}'\n\t\tMessage: {1}", assemblyPath, errorMessage);
			return nullptr;
		}
		...
		if (s_State->Config.EnableDebugging)
		{
			...
			if (!FileSystem::Exists(pdbPath))
			{
				...
				if (!FileSystem::Exists(pdbPath))
				{
					AS_CORE_WARN_TAG("ScriptEngine", "Couldn't find .pdb file for assembly {0}, no debug symbols will be loaded.", assemblyPath.string());
			...
			if (loadDebugSymbols)
			{
				AS_CORE_INFO_TAG("ScriptEngine", "Loading debug symbols from '{0}'", pdbPath.string());
	...
	MonoAssembly* ScriptEngine::LoadMonoAssemblyRuntime(Buffer assemblyData)
	{
		...
		{
			const char* errorMessage = mono_image_strerror(status);
			AS_CORE_ERROR_TAG("ScriptEngine", "Failed to load C# assembly");
			return nullptr;
		}
	...
	{
		AS_CORE_TRACE_TAG("ScriptEngine", "Trying to load core assembly: {}", s_State->Config.CoreAssemblyPath.string());
		if (!FileSystem::Exists(s_State->Config.CoreAssemblyPath))
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Could not load core assembly! Path: {}", s_State->Config.CoreAssemblyPath.string());
			return false;
		}
		...
		if (s_State->ScriptsDomain)
		{
			...
			s_State->NewScriptsDomain = mono_domain_create_appdomain("AspectScriptRuntime", nullptr);
		...
		{
			s_State->ScriptsDomain = mono_domain_create_appdomain("AspectScriptRuntime", nullptr);
		...
		{
			AS_CORE_ERROR_TAG("ScriptEngine", "Failed to load core assembly!");
			return false;
		}
		...
		AS_CORE_INFO_TAG("ScriptEngine", "Successfully loaded core assembly from: {0}", s_State->Config.CoreAssemblyPath);
	...
	void ScriptEngine::UnloadAssembly(AspectRef<AssemblyInfo> assemblyInfo)
	{
	...
	void ScriptEngine::LoadReferencedAssemblies(const AspectRef<AssemblyInfo>& assemblyInfo)
	{
		...
		{
			// Ignore Aspect-ScriptCore and mscorlib, since they're already loaded
			if (assemblyMetadata.Name.find("Aspect-ScriptCore") != std::string::npos)
				continue;
			...
			AS_CORE_INFO_TAG("ScriptEngine", "Loading assembly {0} referenced by {1}", assemblyPath.filename().string(), assemblyInfo->FilePath.filename().string());

			MonoAssembly* assembly = LoadMonoAssembly(assemblyPath);

			if (assembly == nullptr)
			{
				AS_CORE_ERROR_TAG("ScriptEngine", "Failed to load assembly {0} referenced by {1}", assemblyMetadata.Name, assemblyInfo->FilePath);
				continue;
			}
