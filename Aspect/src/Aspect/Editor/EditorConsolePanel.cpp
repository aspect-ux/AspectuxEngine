...
#include "Aspect/Editor/FontAwesome.h"
...
namespace Aspect {
	...
	{
		AS_CORE_ASSERT(s_Instance == nullptr);
		s_Instance = this;
	...
	void EditorConsolePanel::RenderMenu(const ImVec2& size)
	{
		...
		const std::string clearOnPlayText = fmt::format("{} Clear on Play", m_ClearOnPlay ? AS_ICON_CHECK : AS_ICON_TIMES);
...

		{
			...
			if (UI::ColoredButton(AS_ICON_INFO_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Info), textColor, buttonSize))
			...
			if (UI::ColoredButton(AS_ICON_EXCLAMATION_TRIANGLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Warning), textColor, buttonSize))
			...
			if (UI::ColoredButton(AS_ICON_EXCLAMATION_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Error), textColor, buttonSize))
