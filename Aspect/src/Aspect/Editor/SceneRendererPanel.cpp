		...
		if (ImGui::Begin("Scene Renderer", &isOpen) && m_Context)
		{
			...
			*/
			...
			if (UI::PropertyGridHeader("Ambient Occlusion"))
			{
				...
				if (UI::BeginTreeNode("Ground-Truth Ambient Occlusion"))
				{
					...
					if (UI::Property("Enable", options.EnableGTAO))
					{
						...
						Renderer::SetGlobalMacroInShaders("__AS_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(options.EnableGTAO, options.EnableHBAO)));
						Renderer::SetGlobalMacroInShaders("__AS_REFLECTION_OCCLUSION_METHOD", std::to_string((int)options.ReflectionOcclusionMethod));
					}
					if (UI::Property("Bent Normals", options.GTAOBentNormals))
					{
						Renderer::SetGlobalMacroInShaders("__AS_GTAO_COMPUTE_BENT_NORMALS", fmt::format("{}", (int)options.GTAOBentNormals));
			...
			if (UI::PropertyGridHeader("SSR"))
			{
				...
				if (UI::PropertyDropdown("Reflection Occlusion method", aoMethods, 4, methodIndex))
				{
					...
					Renderer::SetGlobalMacroInShaders("__AS_REFLECTION_OCCLUSION_METHOD", fmt::format("{}", options.ReflectionOcclusionMethod));
					Renderer::SetGlobalMacroInShaders("__AS_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(options.EnableGTAO, options.EnableHBAO)));
				}
			...
			*/
			if (UI::PropertyGridHeader("Bloom Settings"))
			{
				...
				//TODO:UI::Image(m_Context->m_BloomDirtTexture, ImVec2(64, 64));
