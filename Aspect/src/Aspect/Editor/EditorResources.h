...
namespace Aspect {
	...
	public:
		...
		inline static Ref<Texture2D> AnimationControllerFileIcon = nullptr;
		...
		// Textures
		inline static Ref<Texture2D> AspectLogoTexture = nullptr;
		...
		static void Init()
		{
			...
			SceneFileIcon = LoadTexture("Aspect.png", "SceneFileIcon", spec);
			...
			// Textures
			AspectLogoTexture = LoadTexture("H_logo.png");
	...
	private:
		...
		static Ref<Texture2D> LoadTexture(const std::filesystem::path& relativePath, const std::string& name, TextureSpecification specification)
		{
			...
			{
				AS_CORE_CRITICAL("Failed to load icon {0}! The file doesn't exist.", path.string());
