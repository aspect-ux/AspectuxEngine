...
namespace Aspect {
	...
	public:
		...
		virtual void SetAsset(const AspectRef<Asset>& asset) = 0;
	...
	public:
		...
		static void SetSceneContext(const AspectRef<Scene>& context);
		...
		static void RegisterEditor(AssetType type)
		{
			...
			AS_CORE_ASSERT(s_Editors.find(type) == s_Editors.end(), "There's already an editor for that asset!");
			s_Editors[type] = CreateScope<T>();
		}

	private:
		static std::unordered_map<AssetType, Scope<AssetEditor>> s_Editors;
		static AspectRef<Scene> s_SceneContext;
	};
