...
#include "Aspect/Asset/AssetExtensions.h"
...
namespace Aspect {
	...
	EditorAssetManager::EditorAssetManager()
	{
		...
		// 4. retrieve folders and project files, write into .asr, then re-register in m_AssetRegistry
		ReloadAssets();
	}
	...
	AspectRef<Asset> EditorAssetManager::GetAsset(AssetHandle assetHandle)
	{
		AS_PROFILE_FUNC();
		//AS_SCOPE_PERF("AssetManager::GetAsset");
	...
	void EditorAssetManager::AddMemoryOnlyAsset(AspectRef<Asset> asset)
	{
	...
	bool EditorAssetManager::ReloadData(AssetHandle assetHandle)
	{
		...
		if (!metadata.IsValid())
		{
			AS_CORE_ERROR("Trying to reload invalid asset");
			return false;
		}

		AspectRef<Asset> asset;
		metadata.IsDataLoaded = AssetImporter::TryLoadData(metadata, asset);
		if (metadata.IsDataLoaded)
		{
			m_LoadedAssets[assetHandle] = asset;
	...
	{
		AS_CORE_INFO("[AssetManager] Loading Asset Registry");

		AS_CORE_WARN("EditorManager::LoadAssetRegistry from ConfigFile:{0}", Project::GetAssetRegistryPath());
		...
		if (!handles)
		{
			AS_CORE_ERROR("[AssetManager] Asset Registry appears to be corrupted!");
		...
		for (auto entry : handles)
		{
			...
			{
				AS_CORE_WARN_TAG("AssetManager", "Mismatch between stored AssetType and extension type when reading asset registry!");
				metadata.Type = GetAssetTypeFromPath(filepath);
			}

			if (!FileSystem::Exists(GetFileSystemPath(metadata)))
			{
				AS_CORE_WARN("[AssetManager] Missing asset '{0}' detected in registry file, trying to locate...", metadata.FilePath);
				...
				for (auto& pathEntry : std::filesystem::recursive_directory_iterator(Project::GetAssetDirectory()))
				{
					...
					if (bestScore > 0)
						AS_CORE_WARN("[AssetManager] Multiple candidates found...");
					...
					AS_CORE_WARN("'{0}' has a score of {1}, best score is {2}", path.string(), score, bestScore);
				...
				{
					AS_CORE_ERROR("[AssetManager] Failed to locate a potential match for '{0}'", metadata.FilePath);
					continue;
				}
				...
				AS_CORE_WARN("[AssetManager] Found most likely match '{0}'", metadata.FilePath);
			}

			if (metadata.Handle == 0)
			{
				AS_CORE_WARN("[AssetManager] AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
		...
		AS_CORE_INFO("[AssetManager] Loaded {0} asset entries", m_AssetRegistry.Count());
	}

	void EditorAssetManager::ProcessDirectory(const std::filesystem::path& directoryPath)
	{
		AS_CORE_INFO("Start ProcessDirectory: {0}", directoryPath.string());
	...
	void EditorAssetManager::WriteRegistryToFile()
	{
		...
		AS_CORE_INFO("Start Write Paths to 'YAML::Emitter out'...");
		...
		for (auto& [handle, entry] : sortedMap)
		{
			...
			out << YAML::EndMap;
			//AS_CORE_INFO("Write {0} to YAML::Emitter", entry.FilePath);
		}
		out << YAML::EndSeq;
		out << YAML::EndMap;
		...
		AS_CORE_INFO("[WriteRegistryToFile] Write Registry to .asr File at : {0}", assetRegistryPath);
