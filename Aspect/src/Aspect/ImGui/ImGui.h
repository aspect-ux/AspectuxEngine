...
#include "Aspect/Editor/AssetEditorPanelInterface.h"
#include "Aspect/ImGui/Colours.h"
#include "Aspect/ImGui/ImGuiUtilities.h"
#include "Aspect/ImGui/ImGuiWidgets.h"
//#include "Aspect/ImGui/ImGuiFonts.h"
#include "Aspect/Utilities/StringUtils.h"
#include "Aspect/Script/ScriptCache.h"

#include "choc/text/choc_StringUtilities.h"

namespace Aspect::UI {

#define AS_MESSAGE_BOX_OK_BUTTON BIT(0)
#define AS_MESSAGE_BOX_CANCEL_BUTTON BIT(1)
#define AS_MESSAGE_BOX_USER_FUNC BIT(2)
	...
	static void ShowMessageBox(const char* title, const std::function<void()>& renderFunction, uint32_t width = 600, uint32_t height = 0)
	{
		...
		messageBoxData.Flags = AS_MESSAGE_BOX_USER_FUNC;
	...
	static void RenderMessageBoxes()
	{
		for (auto& [key, messageBoxData] : s_MessageBoxes)
		{
			...
			{
				if (messageBoxData.Flags & AS_MESSAGE_BOX_USER_FUNC)
				{
					AS_CORE_VERIFY(messageBoxData.UserRenderFunction, "No render function provided for message box!");
				...
				else
				{
					...
					if (messageBoxData.Flags & AS_MESSAGE_BOX_OK_BUTTON)
					{
						if (ImGui::Button("Ok"))
							ImGui::CloseCurrentPopup();

						if (messageBoxData.Flags & AS_MESSAGE_BOX_CANCEL_BUTTON)
							ImGui::SameLine();
					}

					if (messageBoxData.Flags & AS_MESSAGE_BOX_CANCEL_BUTTON && ImGui::Button("Cancel"))
					{
						ImGui::CloseCurrentPopup();
					}
				}
	...
	static bool PropertyMultiline(const char* label, std::string& value)
	{
		...
		{
			s_MultilineBuffer = anew char[s_MultilineBufferSize];
		...
		if (!IsItemDisabled())
		{
			if (ImGui::BeginDragDropTarget())
			{
				...
				if (data)
				{
					...
					AspectRef<Asset> asset = AssetManager::GetAsset<Asset>(assetHandle);
	...
	static bool PropertyEntityReference(const char* label, UUID& entityID, AspectRef<Scene> currentScene)
	{
	...
	static bool PropertyEntityReferenceWithComponents(const char* label, UUID& entityID, AspectRef<Scene> context, bool requiresAllComponents = true)
	{
		bool receivedValidEntity = false;
	...
	static bool PropertyAssetDropdown(const char* label, Ref<TAssetType>& object, const ImVec2& size, AssetHandle* selected)
	{
		...
		AS_CORE_ERROR("Now {0} assets loaded", assets.size());
	...
	static bool DrawFieldValue(AspectRef<Scene> sceneContext, const std::string& fieldName, AspectRef<FieldStorage>& storage)
	{
		if (!storage)
			return false;
		...
		/*if (field->HasAttribute("Aspect.ClampValueAttribute"))
		{
			MonoObject* attr = field->GetAttribute("Aspect.ClampValueAttribute");
			AS_TRY_GET_FIELD_VALUE(min, "Aspect.ClampValueAttribute", "Min", attr);
			AS_TRY_GET_FIELD_VALUE(max, "Aspect.ClampValueAttribute", "Max", attr);
		}*/
		...
		case FieldType::String:
		{
			...
			if (Property(fieldName.c_str(), buffer, 256))
			{
		...
		{
			AssetHandle handle = storage->GetValue<AssetHandle>();
			if (PropertyAssetReference<PhysicsMaterial>(fieldName.c_str(), handle))
			{
				storage->SetValue(handle);
				result = true;
			}
	...
	template<typename T>
	static bool PropertyAssetReferenceArray(const char* label, AssetHandle& outHandle, Ref<ArrayFieldStorage>& arrayStorage, uintptr_t index, intptr_t& elementToRemove, const PropertyAssetReferenceSettings& settings = {})
	{
		bool modified = false;
	...
	static bool PropertyEntityReferenceArray(const char* label, UUID& entityID, AspectRef<Scene> context, AspectRef<ArrayFieldStorage>& arrayStorage, uintptr_t index, intptr_t& elementToRemove)
	{
		bool receivedValidEntity = false;
	...
	static bool DrawFieldArray(AspectRef<Scene> sceneContext, const std::string& fieldName, AspectRef<ArrayFieldStorage>& arrayStorage)
	{
		bool changed = false;
		...
		auto drawStringElement = [&arrayStorage, &elementToRemove](std::string_view indexString, uintptr_t index, const std::string& data)
			{
				ImGuiStyle& style = ImGui::GetStyle();
		...
		for (uint32_t i = 0; i < (uint32_t)length; i++)
		{
			...
			{
				AssetHandle handle = arrayStorage->GetValue<AssetHandle>(i);
				PropertyAssetReferenceArray<PhysicsMaterial>(indexString.c_str(), handle, arrayStorage, i, elementToRemove);
				break;
			}
			case FieldType::Texture2D:
			{
