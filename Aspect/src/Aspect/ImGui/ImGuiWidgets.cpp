...
namespace Aspect::UI
{
				...
				if (ImGui::BeginListBox("##SearchListBox", ImVec2(-FLT_MIN, 0.0f)))
				{
					...
					if (g.NavJustMovedToId != 0)
					{
						if (g.NavJustMovedToId == listID)
						{
							...
							//TODO:ImGui::ActivateItem(listID);
	...
	bool Widgets::EntitySearchPopup(const char* ID, AspectRef<Scene> scene, UUID& selected, bool* cleared /*= nullptr*/, const char* hint /*= "Search Entities"*/, ImVec2 size /*= { 250.0f, 350.0f }*/)
	{
		UI::ScopedColour popupBG(ImGuiCol_PopupBg, UI::ColourWithMultipliedValue(Colours::Theme::background, 1.6f));
