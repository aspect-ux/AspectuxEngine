...
#define AS_PROFILE 1
	...
	#define AS_PROFILE_BEGIN_SESSION(name, filepath) ::Aspect::Instrumentor::Get().BeginSession(name, filepath)
	#define AS_PROFILE_END_SESSION() ::Aspect::Instrumentor::Get().EndSession()
	#define AS_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Aspect::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Aspect::InstrumentationTimer timer##line(fixedName##line.Data)
	#define AS_PROFILE_SCOPE_LINE(name, line) AS_PROFILE_SCOPE_LINE2(name, line)
	#define AS_PROFILE_SCOPE(name) AS_PROFILE_SCOPE_LINE(name, __LINE__)
