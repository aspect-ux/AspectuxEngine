...
/*
#include "Aspect/Platform/Vulkan/VulkanComputePipeline.h"
#include "Aspect/Platform/Vulkan/VulkanMaterial.h"
#include "Aspect/Platform/Vulkan/VulkanRenderer.h"
*/
namespace Aspect {
	...
	SceneRenderer::SceneRenderer(AspectRef<Scene> scene/*, SceneRendererSpecification specification*/)
	...
	{
		AS_SCOPE_TIMER("SceneRenderer::Init");
		...
		m_Renderer2D = AspectRef<Renderer2D>::Create();
		...
		// PreDepth
		{
			//TODO:FrameBufferSpecification preDepthFramebufferSpec;
			//TODO:preDepthFramebufferSpec.DebugName = "PreDepth-Opaque";
			//Linear depth, reversed device depth
			//TODO:preDepthFramebufferSpec.Attachments = { /*ImageFormat::RED32F, */ ImageFormat::DEPTH32FSTENCIL8UINT };
		...
		// Geometry
		{
			...
			pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("AspectPBR_Static");
			...
			pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("AspectPBR_Transparent");
			...
			pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("AspectPBR_Anim");
			pipelineSpecification.DepthOperator = DepthCompareOperator::Equal;
		...
		*/
		...
		/*{
			...
			m_GridShader = Library<Shader>::GetInstance().Get("Grid");
		}*/
	...
	{
		/*
		AspectRef<SceneRenderer> instance = this;
	...
	void SceneRenderer::InitOptions()
	{
		...
		// Special macros are strictly starting with "__AS_"
		Renderer::SetGlobalMacroInShaders("__AS_REFLECTION_OCCLUSION_METHOD", fmt::format("{}", (int)m_Options.ReflectionOcclusionMethod));
		Renderer::SetGlobalMacroInShaders("__AS_AO_METHOD", fmt::format("{}", ShaderDef::GetAOMethod(m_Options.EnableGTAO, m_Options.EnableHBAO)));
		Renderer::SetGlobalMacroInShaders("__AS_GTAO_COMPUTE_BENT_NORMALS", fmt::format("{}", (int)m_Options.GTAOBentNormals));
		*/
	}
	...
	void SceneRenderer::SetScene(AspectRef<Scene> scene)
	{
		AS_CORE_ASSERT(!m_Active, "Can't change scenes while rendering");
		m_Scene = scene;
	}
	...
	void SceneRenderer::BeginScene(const SceneRendererCamera& camera)
	{
		...
		AS_CORE_ASSERT(m_Scene);
		AS_CORE_ASSERT(!m_Active);
		m_Active = true;
...
/*
	...
	{
		AS_CORE_VERIFY(staticMesh->Handle);
		Ref<MeshSource> meshSource = staticMesh->GetMeshSource();
		const auto& submeshData = meshSource->GetSubmeshes();
		for (uint32_t submeshIndex : staticMesh->GetSubmeshes())
		{
...
/*
	void SceneRenderer::ShadowMapPass()
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			{
				AS_CORE_VERIFY(m_MeshTransformMap.find(mk) != m_MeshTransformMap.end());
	...
	void SceneRenderer::BloomCompute()
	{
		...
		m_CommandBuffer->EndTimestampQuery(m_GPUTimeQueries.BloomComputePassQuery);s
	}
