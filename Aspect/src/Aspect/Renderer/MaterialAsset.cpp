...
namespace Aspect {
		...
		: m_Transparent(transparent)
	{
		...
		if (transparent)
			m_Material = Material::Create(Renderer::GetShaderLibrary()->Get("AspectPBR_Transparent"));
		else
			m_Material = Material::Create(Renderer::GetShaderLibrary()->Get("AspectPBR_Static"));

		SetDefaults();
	}

	MaterialAsset::MaterialAsset(AspectRef<Material> material)
	{
		Handle = {};
	...
	{
		AS_CORE_ASSERT(HasMaterial(index));
