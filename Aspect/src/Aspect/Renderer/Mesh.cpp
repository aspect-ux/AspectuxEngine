...
namespace Aspect
{
	...
	void Mesh::LoadModel(const std::string& path)
	{
		...
		{
			AS_CORE_ERROR("ERROR::ASSIMP::{0}", importer.GetErrorString());
			return;
		}
		...
		if (bAnimated)
		{
			for (size_t boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
			{
				...
				for (int weightIndex = 0; weightIndex < numWeights; ++weightIndex)
				{
					...
					AS_CORE_ASSERT(vertexId <= vertices.size());
					for (int i = 0; i < MAX_BONE_INFLUENCE; ++i)
					{
	...
	std::optional<std::vector<MaterialTexture>> Mesh::loadMaterialTextures(aiMaterial* mat, aiTextureType type, uint32_t& subMeshIndex)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				...
				{
					AS_CORE_WARN("Load Texture failed!");
					texture.texture2d = Texture2D::Create(AssetManager::GetFullPath("Assets/Textures/DefaultTexture.png").string());
				}
