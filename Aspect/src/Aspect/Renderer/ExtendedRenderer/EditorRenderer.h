...
namespace Aspect { enum class EGizmoOperation; }
...
namespace Aspect
{
	...
	*/
	class EditorRenderer : public RefCounted
	{
	public:
		/**
		...
		* @param p_context
		*/
		EditorRenderer(/*Context& p_context*/ AspectRef<Scene> scene);
		...
		* @param p_material
		*/
		//void PreparePickingMaterial(ECS::Actor& p_actor, Material& p_material);

		/**
		* Calculate the model matrix for a camera attached to the given actor
		* @param p_actor
		*/
		//OvMaths::FMatrix4 CalculateCameraModelMatrix(ECS::Actor& p_actor);
		...
		* @param p_camera
		*/
		void RenderScene(const glm::vec3& p_cameraPosition, EditorCamera& p_camera/*, const OvRendering::Data::Frustum* p_customFrustum = nullptr*/);
		...
		*/
		//void RenderGizmo(const glm::vec3& p_position, const OvMaths::FQuaternion& p_rotation, EGizmoOperation p_operation, bool p_pickable, int p_highlightedAxis = -1);

		/**
		* Render a model to the stencil buffer
		*/
		void RenderModelToStencil(const glm::mat4x4& p_worldMatrix, Mesh& p_model);
		...
		* @param p_width
		*/
		//void RenderModelOutline(const OvMaths::FMatrix4& p_worldMatrix, OvRendering::Model& p_model, float p_width);
		...
		* @param p_camera
		*/
		void RenderCameraPerspectiveFrustum(std::pair<uint16_t, uint16_t>& p_size, CameraComponent& p_camera);
		...
		* @param p_camera
		*/
		void RenderCameraOrthographicFrustum(std::pair<uint16_t, uint16_t>& p_size, CameraComponent& p_camera);

		/**
		* Render the camera frustum
		*/
		void RenderCameraFrustum(CameraComponent& p_camera);
		...
		*/
		//void RenderBoundingSpheres(ECS::Components::CModelRenderer& p_modelRenderer);

		/**
		* Render model
		*/
		//void RenderModelAsset(OvRendering::Model& p_model);

		/**
		* Render texture
		*/
		void RenderTextureAsset(Texture& p_texture);
		...
		* @param p_scene
		*/
		//void UpdateLightsInFrustum(SceneSystem::Scene& p_scene, const OvRendering::Data::Frustum& p_frustum);

	private:
		//Context& m_context;
		AspectRef<Material> m_defaultMaterial;
		/*
